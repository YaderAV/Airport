...
public class PassengerValidator {
    ...
    public PassengerValidator(Map<Long, Passenger> existingPassengers, boolean isUpdate) {
    ...
    public ValidationResult validate(Passenger p) {
        ...
        // Solo verificar duplicados si es creación
        if (!isUpdate && existingPassengers.containsKey(p.getId())) {
            result.addError("Ya existe un pasajero con este ID");
        }

        if (p.getFirstname() == null || p.getFirstname().isBlank()) {
            result.addError("Nombre vacío.");
        }

        if (p.getLastname() == null || p.getLastname().isBlank()) {
            result.addError("Apellido vacío.");
        }
        ...
        if (p.getPhone() <= 0 || String.valueOf(p.getPhone()).length() > 11) {
            result.addError("Teléfono inválido. Máximo 11 dígitos.");
        }

        if (p.getCountryPhoneCode() <= 0 || String.valueOf(p.getCountryPhoneCode()).length() > 3) {
            result.addError("Código telefónico inválido. Máximo 3 dígitos y mayor que 0.");
        }
