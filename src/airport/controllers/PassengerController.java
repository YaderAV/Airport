...
public class PassengerController {
    ...
    public Object[][] getPassengerRows() {
        PassengerController PassengerController = new PassengerController(passengers, storage, flights, passengerUpdateService);
        PassengerRepository pr = new PassengerRepository(PassengerController);
        List<Passenger> passengers = (List<Passenger>) pr.getAllPassengers();
        Object [][] rows = new Object[passengers.size()][];
        for(int i= 0; i<passengers.size(); i++){
            rows[i] = PassengerTableModelAdapter.toRow(passengers.get(i));
        }
        return rows;
    }
